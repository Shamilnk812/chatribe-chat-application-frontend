...
import UserProfileCardSkeletonAnimation from './Animation/UserProfileCardSkeltonAnimation'
import NoUsersFound from './EmptyData/NoUsersFound'





const UserProfileCard = ({ users, setUsers, searchQuery, setSearchQuery, fetchAllUsers, userId, loading }) => {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 pt-6">
                    ...
                    {loading ? (
                        Array(8).fill(0).map((_, index) => (
                            <UserProfileCardSkeletonAnimation key={`skeleton-${index}`} />
                        ))
                    ) : (
                        <>
